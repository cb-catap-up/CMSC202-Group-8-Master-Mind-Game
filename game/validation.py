...
def validateInput(guess_string, valid_colors):
    ...
    try: 
        ...
        if len(guess_clean) != 4:
            return False, f"\nPlease enter exactly 4 characters. You entered {len(guess_clean)} characters.", []
    except AttributeError:
        return False, "\nInvalid input: Please enter a valid string.", []
    except Exception as e:
        return False, f"\nError occurred while validating input: {str(e)}", []
    ...
    try: 
        if invalid_colors:
            return False, f"\nInvalid color(s): {', '.join(invalid_colors)}. Valid colors are: {', '.join(valid_colors)}", []
    except (TypeError, AttributeError) as e:
        return False, f"\nError processing invalid colors: {str(e)}", []
    except Exception as e:
        return False, f"\nUnexpected error in color validation: {str(e)}", []
